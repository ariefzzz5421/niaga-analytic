...
export function ProfitabilityCalculator() {
  ...
  return (
    <section className="mt-10" id="profit-calculator">
      <div className="mb-5">
        <p className="text-xs font-medium uppercase tracking-[0.14em] text-[var(--text-muted)]">Marketplace Profit Calculator</p>
        <h2 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Hitung profit sebelum listing.</h2>
        <p className="mt-2 max-w-2xl text-sm text-[var(--text-secondary)]">Masukkan economics produk sekali, lalu bandingkan settlement, profit, margin, dan harga jual target di setiap channel.</p>
      </div>

      <div className="grid gap-4 lg:grid-cols-[minmax(0,360px)_minmax(0,1fr)]">
        <div className="card h-fit p-4 sm:p-5">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-1">
            <MoneyInput label="HPP / Cost of Goods" value={hpp} onChange={setHpp} />
            ...
            <MoneyInput label="Packaging / order" value={packaging} onChange={setPackaging} />
            <MoneyInput label="Ads / CAC per order" value={ads} onChange={setAds} />
        ...
        <div className="min-w-0 space-y-3">
          {rows.map(({ platform, result, targetPrice }, index) => (
            <article key={platform.id} className="card overflow-hidden p-4 sm:p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div><div className="flex items-center gap-2"><span className="grid size-8 place-items-center rounded-lg bg-[var(--surface-3)] text-xs font-bold">{platform.label.slice(0, 1)}</span><h3 className="text-sm font-semibold">{platform.label}</h3>{index === 0 ? <span className="rounded-full bg-[var(--accent-soft)] px-2 py-0.5 text-[10px]">Best profit</span> : null}</div><p className="mt-2 text-[11px] text-[var(--text-muted)]">{platform.note}</p></div>
                <div className="text-right"><p className="text-[10px] uppercase tracking-wide text-[var(--text-muted)]">Net profit</p><p className={`mt-1 text-lg font-semibold ${result.netProfit >= 0 ? "text-[var(--good)]" : "text-red-400"}`}>{rupiah(result.netProfit)}</p></div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                <Metric label="Platform fee" value={rupiah(result.marketplaceFees)} />
                <Metric label="Settlement" value={rupiah(result.netSettlement)} />
                <Metric label="Net margin" value={`${result.netMargin.toFixed(1)}%`} />
                <Metric label={`Target ${target}%`} value={rupiah(targetPrice)} />
              </div>
            </article>
          ))}
        </div>
...
function MoneyInput({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="text-xs text-[var(--text-secondary)]">{label}<div className="relative mt-1.5"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-[var(--text-muted)]">Rp</span><input className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface-2)] py-3 pl-9 pr-3 text-base outline-none" inputMode="numeric" type="number" min={0} value={value} onChange={(e) => onChange(Math.max(0, Number(e.target.value)))} /></div></label>;
}
