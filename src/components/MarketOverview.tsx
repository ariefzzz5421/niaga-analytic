...
function Logo({ id, label, compact = false }: { id: MarketId; label: string; compact?: boolean }) {
    ...
    return (
      <span className="flex items-center -space-x-1.5" aria-label="TikTok Shop and Tokopedia logos">
        <span className={`${size} grid place-items-center overflow-hidden rounded-xl bg-black p-1.5 ring-2 ring-[var(--surface-1)]`}>
          <img src="/marketplaces/tiktok.png" alt="TikTok Shop" className="h-full w-full object-contain" />
        </span>
        <span className={`${compact ? "h-8 w-11" : "h-10 w-14"} grid place-items-center overflow-hidden rounded-xl bg-white p-0.5 ring-2 ring-[var(--surface-1)]`}>
          <img src="/marketplaces/tokopedia-icon.png" alt="Tokopedia" className="h-full w-full object-contain" />
        </span>
      </span>
    );
  }
  ...
  return (
    <span className={`${size} grid place-items-center overflow-hidden rounded-xl bg-white p-1.5`}>
      <img src={LOGO[id]} alt={`${label} logo`} className="h-full w-full object-contain" />
    </span>
  );
}

function TrafficLogo({ site, compact = false }: { site: TrafficSite; compact?: boolean }) {
  const wide = site.id === "tokopedia";
  const box = compact ? (wide ? "h-7 w-10" : "h-7 w-7") : wide ? "h-10 w-14" : "h-10 w-10";
    ...
    <div className="mt-6 space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface-1)]">
        <div className="border-b border-[var(--border)] p-4 sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="text-[10px] font-medium uppercase tracking-[0.16em] text-[var(--text-muted)]">Indonesia E-Commerce traffic snapshot</div>
              <h2 className="mt-2 text-lg font-semibold sm:text-xl">Top-10 e-commerce / shopping traffic</h2>
              <p className="mt-1 max-w-2xl text-xs leading-relaxed text-[var(--text-secondary)]">Organic-traffic estimates for the top-10 sites in the supplied snapshot. This is a traffic comparison, not GMV market share.</p>
            </div>
            <div className="inline-flex w-fit rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-1">
              <button type="button" onClick={() => setTrafficMode("traffic")} className={`rounded-lg px-3 py-2 text-[11px] font-medium ${trafficMode === "traffic" ? "bg-[var(--surface-3)] text-white" : "text-[var(--text-muted)]"}`}>Traffic</button>
              <button type="button" onClick={() => setTrafficMode("share")} className={`rounded-lg px-3 py-2 text-[11px] font-medium ${trafficMode === "share" ? "bg-[var(--surface-3)] text-white" : "text-[var(--text-muted)]"}`}>Top-10 share %</button>
            </div>
          </div>

          <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
            <div className="rounded-xl bg-[var(--surface-2)] p-3"><p className="text-[9px] uppercase tracking-wide text-[var(--text-muted)]">Top-10 organic traffic</p><p className="mt-1 text-base font-semibold sm:text-lg">{TOP10_TRAFFIC_TOTAL_M.toFixed(1)}M</p></div>
            <div className="rounded-xl bg-[var(--surface-2)] p-3"><p className="text-[9px] uppercase tracking-wide text-[var(--text-muted)]">Top 4 share</p><p className="mt-1 text-base font-semibold sm:text-lg">{TOP4_TRAFFIC_SHARE.toFixed(1)}%</p></div>
            <div className="rounded-xl bg-[var(--surface-2)] p-3"><p className="text-[9px] uppercase tracking-wide text-[var(--text-muted)]">#1 traffic</p><p className="mt-1 text-base font-semibold sm:text-lg">Shopee 53.0M</p></div>
          </div>
        </div>

        <div className="grid gap-0 xl:grid-cols-[1.05fr_.95fr]">
          <div className="divide-y divide-[var(--border)] p-3 sm:p-4">
            <div className="hidden grid-cols-[44px_minmax(160px,1fr)_110px_110px] gap-3 px-3 pb-2 text-[9px] font-medium uppercase tracking-wide text-[var(--text-muted)] sm:grid">
              <span>Rank</span><span>Site</span><span className="text-right">Traffic est.</span><span className="text-right">Top-10 share</span>
            </div>
            {TOP10_ECOMMERCE_TRAFFIC.map((site) => {
              ...
              return (
                <button
                  key={site.id}
                  type="button"
                  onClick={() => setSelectedTrafficId(site.id)}
                  onMouseEnter={() => setSelectedTrafficId(site.id)}
                  className={`w-full rounded-xl px-3 py-3 text-left transition ${isActive ? "bg-[var(--surface-2)]" : "hover:bg-[var(--surface-2)]/70"}`}
                >
                  <div className="grid grid-cols-[36px_minmax(0,1fr)_auto] items-center gap-3 sm:grid-cols-[44px_minmax(160px,1fr)_110px_110px]">
                    ...
                    <span className="flex min-w-0 items-center gap-3">
                      <TrafficLogo site={site} compact />
                      <span className="truncate text-xs font-medium sm:text-sm">{site.label}</span>
                    </span>
                    ...
                    <span className="hidden text-right text-xs font-semibold tabular-nums sm:block">{site.trafficM.toFixed(1)}M</span>
                    <span className="hidden text-right text-xs font-semibold tabular-nums sm:block">{site.share.toFixed(1)}%</span>
                  </div>
                  <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--surface-3)]">
                    <div className="h-full rounded-full transition-all duration-300" style={{ width: `${Math.max(2, width)}%`, background: site.accent }} />
                  </div>
                </button>
              );
            })}
          ...
          <aside className="border-t border-[var(--border)] bg-[var(--surface-2)] p-4 sm:p-5 xl:border-l xl:border-t-0">
            <div className="h-[430px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={TOP10_ECOMMERCE_TRAFFIC} layout="vertical" margin={{ top: 8, right: 12, left: 8, bottom: 8 }}>
                  <CartesianGrid stroke="rgba(255,255,255,0.06)" horizontal={false} />
                  <XAxis type="number" tick={{ fill: "#7f818a", fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis type="category" dataKey="label" width={76} tick={{ fill: "#a5a7af", fontSize: 10 }} axisLine={false} tickLine={false} />
                  <Tooltip
                    contentStyle={{ background: "#111216", border: "1px solid #2a2c33", borderRadius: 12, fontSize: 12 }}
                    itemStyle={{ color: "#fff" }}
                    formatter={(value, _name, item) => {
                      const site = item?.payload as TrafficSite | undefined;
                      return [trafficMode === "traffic" ? `${Number(value).toFixed(1)}M visits` : `${Number(value).toFixed(1)}%`, site?.label ?? "Traffic"];
                    }}
                  />
                  <Bar dataKey={trafficMode === "traffic" ? "trafficM" : "share"} radius={[0, 7, 7, 0]}>
                    {TOP10_ECOMMERCE_TRAFFIC.map((site) => <Cell key={site.id} fill={site.accent} opacity={selectedTrafficId === site.id ? 1 : 0.5} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>

            <div className="mt-3 rounded-xl border border-[var(--border)] bg-[var(--surface-1)] p-4">
              <div className="flex items-center gap-3">
                <TrafficLogo site={activeTraffic} />
                <div className="min-w-0"><p className="truncate text-sm font-semibold">#{activeTraffic.rank} {activeTraffic.label}</p><p className="text-[10px] text-[var(--text-muted)]">Organic traffic snapshot</p></div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <div className="rounded-xl bg-[var(--surface-2)] p-3"><p className="text-[9px] uppercase tracking-wide text-[var(--text-muted)]">Traffic est.</p><p className="mt-1 text-lg font-semibold">{activeTraffic.trafficM.toFixed(1)}M</p></div>
                <div className="rounded-xl bg-[var(--surface-2)] p-3"><p className="text-[9px] uppercase tracking-wide text-[var(--text-muted)]">Top-10 share</p><p className="mt-1 text-lg font-semibold">{activeTraffic.share.toFixed(1)}%</p></div>
              </div>
            </div>
