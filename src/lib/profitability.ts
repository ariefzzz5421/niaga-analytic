...
export function calculateProfit(input: ProfitInput, fees: FeeRule[]): ProfitResult {
  ...
  const breakdown = fees.map((fee) => ({
    label: fee.label,
    amount: Math.round(grossRevenue * (fee.rate ?? 0) + (fee.fixed ?? 0) * quantity),
  }));
  const affiliate = Math.round(grossRevenue * Math.max(0, input.affiliateRate ?? 0));
  ...
  return {
    ...
    effectiveTakeRate: grossRevenue ? (marketplaceFees / grossRevenue) * 100 : 0,
    breakdown,
  };
}
...
export function findTargetPrice(input: Omit<ProfitInput, "sellingPrice">, fees: FeeRule[], targetMargin = 0): number {
  let low = 0;
  let high = Math.max(1000, input.hpp * 10 + 1_000_000);
  for (let i = 0; i < 64; i += 1) {
    const mid = (low + high) / 2;
    const result = calculateProfit({ ...input, sellingPrice: mid }, fees);
    if (result.netMargin / 100 >= targetMargin) high = mid;
    else low = mid;
  }
